...
$outDataName = "$outPrefix" + "_" + "$opMode" + "_Data" + "$trimDesc" + "_.csv"
...
foreach ( $i in $checksMembersIndex ) {
    ...
    If ( $dateObject -gt $newerThresholdDateTime ) {
        $checks[$i].Newer = $true
    }
    Else {
...
#ignore "help" file
$checks = $checks | Where {$_.Name -notlike "$ignoreSuffix"}
